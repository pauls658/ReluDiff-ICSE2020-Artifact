...
struct NNet *load_network(const char* filename, int target)
{
    ...
    /* Copy weights into matrix structs. */
...
void load_inputs(int PROPERTY, int inputSize, float *u, float *l, struct NNet *nnet)
{
    ...
    if (!MNIST_3PIX) {
        // MNIST properties
        printf("Global MNIST perturbation\n");
        ...
        printf("3 pixel MNIST perturbation\n");
        if (PROPERTY >= 400 && PROPERTY <= 499) {
            int num_pixels = 3;
            int testNum = PROPERTY - 400;
            for (int i = 0; i < 784; i++) {
...
int forward_prop(struct NNet *network, struct Matrix *input, struct Matrix *output)
{
    ...
    for(layer=0;layer<numLayers;layer++){
        ...
        if(layer<numLayers-2){ // don't perform ReLU for last _and_ 2nd to last layer
            relu(&A);
        }
                                ...
                                struct Interval *grad)
{
    ...
    int equationSize = (nnet->inputSize + 1)*nnet->maxLayerSize;
    float *equation_lower = malloc(equationSize* sizeof(float));
    float *equation_upper = malloc(equationSize* sizeof(float));
    float *new_equation_lower = malloc(equationSize* sizeof(float));
    float *new_equation_upper = malloc(equationSize* sizeof(float));
    float *p = malloc(nnet->maxLayerSize*nnet->maxLayerSize* sizeof(float));
    float *n = malloc(nnet->maxLayerSize*nnet->maxLayerSize* sizeof(float));
    ...
    struct Interval new_equation_interval = {
    ...
    for (layer=0;layer<(numLayers);layer++) {
        ...
        matmul(&equation_interval.upper_matrix, &pos_weights,\
                        &new_equation_interval.upper_matrix);
        matmul_with_bias(&equation_interval.lower_matrix, &neg_weights,\
                        &new_equation_interval.upper_matrix);

        matmul(&equation_interval.lower_matrix, &pos_weights,\
                        &new_equation_interval.lower_matrix);
        matmul_with_bias(&equation_interval.upper_matrix, &neg_weights,\
                        &new_equation_interval.lower_matrix);
        ...
        for (i=0; i < nnet->layerSizes[layer+1]; i++)
        {
            ...
            if (layer < (numLayers - 1)) {
                if (layer == numLayers - 2) {
                    // The second to last layer subtracts the two results
                    // so don't perform Relu. Also, the gradient of both
                    // equations is 1.
                    R[layer][i] = 2;
                } else if (tempVal_upper <= 0.0) {
                    tempVal_upper = 0.0;
        ...
        equation_interval.lower_matrix.row = equation_interval.upper_matrix.row\
                    =  new_equation_interval.lower_matrix.row;
        equation_interval.lower_matrix.col = equation_interval.upper_matrix.col\
                    = new_equation_interval.lower_matrix.col;
    }
