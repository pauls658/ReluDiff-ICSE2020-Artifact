...
elina_interval_t * subtract_output_neurons(elina_manager_t* man, elina_abstract0_t* element, elina_dim_t y, elina_dim_t x, bool use_area_heuristic) {
	fppoly_t *fp = fppoly_of_abstract0(element);
	fppoly_internal_t * pr = fppoly_init_from_manager(man,ELINA_FUNID_ASSIGN_LINEXPR_ARRAY);
	...
	double ub = get_ub_using_previous_layers(man, fp, sub, fp->numlayers, use_area_heuristic);
